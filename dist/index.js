        ...
        try {
          ...
          if (!tags) {
            if (fallbackToNoPrefixSearch2) {
              core2.info(`No tags were found with the prefix '${prefix}'.  Falling back to searching with no prefix...`);
              tags = git("tag");
            }
            if (!tags) {
...
`);
          ...
          core2.setFailed(`An error occurred listing the tags for the repository: ${error.message}`);
        }
      },
...
var require_version2 = __commonJS({
  "src/version.js"(exports2, module2) {
    ...
    function getPriorReleaseCommit(tagPrefix2, fallbackToNoPrefixSearch2) {
      ...
      const semverReleaseTags = tags.map((tag) => ({
        tag,
        semverValue: semver.clean(tag.startsWith(tagPrefix2) ? tag.slice(tagPrefix2.length) : tag, true)
      })).filter((tagObj) => tagObj.semverValue !== null && semver.prerelease(tagObj.semverValue) === null).sort((a, b) => semver.rcompare(a.semverValue, b.semverValue));
    ...
    function determineReleaseTypeFromGitLog(baseCommit, finalCommit) {
      ...
      for (let i = 0; i < commitObjects.length; i++) {
        ...
        if (commitPatterns.major.some((pattern) => pattern.test(commitObj.rawBody) || pattern.test(commitObj.commitNotes))) {
          releaseType = "major";
        ...
        if (commitPatterns.minor.some((pattern) => pattern.test(commitObj.rawBody) || pattern.test(commitObj.commitNotes))) {
          releaseType = "minor";
    ...
    function dateToPreReleaseComponent(input) {
      ...
      return `${twoDigit(year)}${twoDigit(month)}${twoDigit(day)}${twoDigit(hour)}${twoDigit(minute)}${twoDigit(second)}`;
    }
...
Prior release version: ${priorReleaseVersion}`);
    ...
    function nextPrereleaseVersion2(label, defaultReleaseType2, tagPrefix2, fallbackToNoPrefixSearch2) {
      let baseCommit;
      try {
        baseCommit = getPriorReleaseCommit(tagPrefix2, fallbackToNoPrefixSearch2);
      } catch (error) {
...
var includeMajorRelease = core.getBooleanInput("include-major-release");
...
async function run() {
  try {
    ...
    if (calculatePrereleaseVersion) {
      ...
      versionToBuild = nextPrereleaseVersion(prereleaseLabel, defaultReleaseType, tagPrefix, fallbackToNoPrefixSearch);
    } else {
    ...
    let versionToBuildNoPrefix = tagPrefix.length > 0 ? versionToBuild.substring(tagPrefix.length) : versionToBuild;
    ...
    core.setFailed(`An error occurred calculating the next ${versionTxt} version: ${error.message}`);
  }
}
run();
/*!
